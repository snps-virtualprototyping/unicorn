    ...
    target_ulong val)
{
    ...
    // unicorn trace_pmpcfg_csr_write(env->mhartid, reg_index, val);
...
target_ulong pmpcfg_csr_read(CPURISCVState *env, uint32_t reg_index)
{
    ...
    // unicorn trace_pmpcfg_csr_read(env->mhartid, reg_index, cfg_val);
...
void pmpaddr_csr_write(CPURISCVState *env, uint32_t addr_index,
    target_ulong val)
{
    // unicorn trace_pmpaddr_csr_write(env->mhartid, addr_index, val);
...
target_ulong pmpaddr_csr_read(CPURISCVState *env, uint32_t addr_index)
{
    ...
    if (addr_index < MAX_RISCV_PMPS) {
        ...
        // unicorn trace_pmpaddr_csr_read(env->mhartid, addr_index, val);
    } else {
        qemu_log_mask(LOG_GUEST_ERROR,
...
void mseccfg_csr_write(CPURISCVState *env, target_ulong val)
{
    ...
    // unicorn trace_mseccfg_csr_write(env->mhartid, val);
...
{
    // unicorn trace_mseccfg_csr_read(env->mhartid, env->mseccfg);
    return env->mseccfg;
}
