...
static void gen_vec_rsub8_i64(TCGContext *s, TCGv_i64 d, TCGv_i64 a, TCGv_i64 b)
{
...
{
    tcg_gen_sub_i32(s, ret, arg2, arg1);
}

static void gen_rsub_i64(TCGContext *s, TCGv_i64 ret, TCGv_i64 arg1, TCGv_i64 arg2)
{
    tcg_gen_sub_i64(s, ret, arg2, arg1);
}

static void gen_rsub_vec(TCGContext *s, unsigned vece, TCGv_vec r, TCGv_vec a, TCGv_vec b)
{
    tcg_gen_sub_vec(s, vece, r, b, a);
}
                               ...
                               TCGv_i64 c, uint32_t oprsz, uint32_t maxsz)
{
    ...
    tcg_gen_gvec_2s(s, dofs, aofs, oprsz, maxsz, c, &rsub_op[vece]);
}
...
static void tcg_gen_gvec_rsubi(TCGContext *s, unsigned vece, uint32_t dofs, uint32_t aofs,
                               int64_t c, uint32_t oprsz, uint32_t maxsz)
{
    TCGv_i64 tmp = tcg_const_i64(s, c);
    tcg_gen_gvec_rsubs(s, vece, dofs, aofs, tmp, oprsz, maxsz);
    ...
    if (vext_check_isa_ill(s) &&
        ...
        if (s->vl_eq_vlmax) {
            tcg_gen_gvec_dup_i64(tcg_ctx, s->sew, vreg_ofs(s, a->rd),
                                 MAXSZ(s), MAXSZ(s), tcg_ctx->cpu_fpr_risc[a->rs1]);
        } else {
            TCGv_ptr dest;
            TCGv_i32 desc;
            uint32_t data = FIELD_DP32(0, VDATA, LMUL, s->lmul);
...
static bool trans_vrgather_vx(DisasContext *s, arg_rmrr *a)
{
    ...
    if (a->vm && s->vl_eq_vlmax) {
        ...
        if (a->rs1 == 0) {
            ...
            vec_element_loadx(s, dest, a->rs2, tcg_ctx->cpu_gpr_risc[a->rs1], vlmax);
        }
