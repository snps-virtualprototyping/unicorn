...
#else
    tcg_gen_mov_i64(s->uc->tcg_ctx, clean, addr);
#endif
    return clean;
}
...
static inline bool use_goto_tb(DisasContext *s, int n, uint64_t dest)
{
     ...
     */
    if (s->base.singlestep_enabled || s->ss_active || (tb_cflags(s->base.tb) & CF_LAST_IO)) {
        return false;
    }
...
static inline void gen_goto_tb(DisasContext *s, int n, uint64_t dest)
{
    ...
    if (use_goto_tb(s, n, dest)) {
        gen_a64_set_pc_im(s, dest); // SNPS moved
                              ...
                              gen_helper_gvec_3_ptr *fn)
{
    ...
    TCGv_ptr fpst = fpstatus_ptr(tcg_ctx, is_fp16 ? FPST_FPCR_F16 : FPST_FPCR);
                       ...
                       unsigned int crn, unsigned int crm, unsigned int rt)
{
    ...
    // SNPS added: always set PC to current address to allow IO register
    // callbacks to read it correctly (moved from below)
    gen_a64_set_pc_im(s, s->pc_curr);
    ...
    if (ri->accessfn) {
        ...
        //gen_a64_set_pc_im(s, s->pc_curr); // SNPS removed
    ...
    case ARM_CP_DC_ZVA:
        ...
        if (s->mte_active[0]) {
            ...
            gen_helper_mte_check_zva(tcg_ctx, tcg_rt, tcg_ctx->cpu_env, t_desc, cpu_reg(s, rt));
...
static void disas_exc(DisasContext *s, uint32_t insn)
{
    ...
    case 2:
         ...
         */
        if (semihosting_enabled(s->uc) && imm16 == 0xf000) { // SNPS changed
#ifndef CONFIG_USER_ONLY
                                      ...
                                      int rn, int size)
{
    ...
    } else if (s->base.tb->cflags & CF_PARALLEL) { // SNPS changed
                         ...
                         unsigned int rn, unsigned int rd)
{
    ...
    if (sf) {
        tcg_gen_bswap64_i64(tcg_ctx, tcg_rd, tcg_rn);
        tcg_gen_rotri_i64(tcg_ctx, tcg_rd, tcg_rd, 32);
    } else {
        tcg_gen_bswap32_i64(tcg_ctx, tcg_rd, tcg_rn, TCG_BSWAP_OZ);
    }
}
                              ...
                              bool cmp_with_zero, bool signal_all_nans)
{
    ...
    TCGv_ptr fpst = fpstatus_ptr(tcg_ctx, size == MO_16 ? FPST_FPCR_F16 : FPST_FPCR);
...
{
    TCGContext *tcg_ctx = s->uc->tcg_ctx;
    void (*gen_fpst)(TCGContext *, TCGv_i32, TCGv_i32, TCGv_ptr);
...
{
    TCGContext *tcg_ctx = s->uc->tcg_ctx;
    void (*gen_fpst)(TCGContext *, TCGv_i64, TCGv_i64, TCGv_ptr);
                           ...
                           int rd, int rn, int dtype, int ntype)
{
    ...
    case 0x0:
    {
        ...
        if (dtype == 1) {
            ...
            TCGv_ptr fpst = fpstatus_ptr(tcg_ctx, FPST_FPCR);
    ...
    {
        TCGv_i32 tcg_rn = read_fp_sreg(s, rn);
        TCGv_ptr tcg_fpst = fpstatus_ptr(tcg_ctx, FPST_FPCR);
                                ...
                                int rd, int rn, int rm, int ra)
{
    ...
    TCGv_i32 tcg_op1, tcg_op2, tcg_op3;
    TCGv_i32 tcg_res = tcg_temp_new_i32(tcg_ctx);
    TCGv_ptr fpst = fpstatus_ptr(tcg_ctx, FPST_FPCR_F16);
                           ...
                           bool itof, int rmode, int scale, int sf, int type)
{
    ...
    tcg_fpstatus = fpstatus_ptr(tcg_ctx, type == 3 ? FPST_FPCR_F16 : FPST_FPCR);
...
static void handle_fjcvtzs(DisasContext *s, int rd, int rn)
{
    ...
    TCGv_ptr fpstatus = fpstatus_ptr(tcg_ctx, FPST_FPCR);
...
static void disas_simd_scalar_pairwise(DisasContext *s, uint32_t insn)
{
    ...
    case 0x2f: /* FMINP */
        ...
        fpst = fpstatus_ptr(tcg_ctx, size == MO_16 ? FPST_FPCR_F16 : FPST_FPCR);
        break;
    default:
        unallocated_encoding(s);
        return;
...
{
    TCGContext *tcg_ctx = s->uc->tcg_ctx;
    TCGv_ptr tcg_fpst = fpstatus_ptr(tcg_ctx, size == MO_16 ? FPST_FPCR_F16 : FPST_FPCR);
                                         ...
                                         int immh, int immb, int rn, int rd)
{
    ...
    tcg_fpstatus = fpstatus_ptr(tcg_ctx, size == MO_16 ? FPST_FPCR_F16 : FPST_FPCR);
    gen_helper_set_rmode(tcg_ctx, tcg_rmode, tcg_rmode, tcg_fpstatus);
    fracbits = (16 << size) - immhb;
    tcg_shift = tcg_const_i32(tcg_ctx, fracbits);
...
static void disas_simd_scalar_two_reg_misc(DisasContext *s, uint32_t insn)
{
    ...
    if (is_fcvt) {
        tcg_rmode = tcg_const_i32(tcg_ctx, arm_rmode_to_sf(rmode));
        tcg_fpstatus = fpstatus_ptr(tcg_ctx, FPST_FPCR);
        gen_helper_set_rmode(tcg_ctx, tcg_rmode, tcg_rmode, tcg_fpstatus);
    } else {
        tcg_rmode = NULL;
        tcg_fpstatus = NULL;
                       ...
                       bool is_q, int size, int rn, int rd)
{
    ...
    if (size == 0) {
        ...
        for (i = 0; i < groups; i++) {
            ...
            switch (grp_size) {
            case MO_16:
                tcg_gen_bswap16_i64(tcg_ctx, tcg_tmp, tcg_tmp, TCG_BSWAP_IZ);
                break;
            case MO_32:
                tcg_gen_bswap32_i64(tcg_ctx, tcg_tmp, tcg_tmp, TCG_BSWAP_IZ);
                break;
            case MO_64:
...
static void disas_simd_two_reg_misc_fp16(DisasContext *s, uint32_t insn)
{
    ...
    if (need_rmode || need_fpst) {
        tcg_fpstatus = fpstatus_ptr(tcg_ctx, FPST_FPCR_F16);
    }
...
static void disas_simd_indexed(DisasContext *s, uint32_t insn)
{
    ...
    if (is_fp) {
        fpst = fpstatus_ptr(tcg_ctx, is_fp16 ? FPST_FPCR_F16 : FPST_FPCR);
    } else {
        fpst = NULL;
    }
...
{
    tcg_gen_rotli_i64(s, d, m, 1);
    tcg_gen_xor_i64(s, d, d, n);
}

static void gen_rax1_vec(TCGContext *s, unsigned vece, TCGv_vec d, TCGv_vec n, TCGv_vec m)
{
    tcg_gen_rotli_vec(s, vece, d, m, 1);
    tcg_gen_xor_vec(s, vece, d, d, n);
}
                   ...
                   uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    ...
    tcg_gen_gvec_3(s, rd_ofs, rn_ofs, rm_ofs, opr_sz, max_sz, &op);
}
