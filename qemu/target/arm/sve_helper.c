                                      ...
                                      uintptr_t retaddr)
{
    ...
    if (mem_off >= 0) {
        cpu_check_watchpoint(env_cpu(env), addr + mem_off, msize,
                             info->page[0].attrs, wp_access, retaddr, ~0ul); // SNPS changed
    }
                ...
                if (likely(in_page >= msize)) {
                    if (unlikely(info.flags & TLB_WATCHPOINT)) {
                        cpu_check_watchpoint(env_cpu(env), addr, msize,
                                             info.attrs, BP_MEM_READ, retaddr, ~0ul); // SNPS changed
                    }
                    if (mtedesc && arm_tlb_mte_tagged(&info.attrs)) {
                        mte_check1_(env, mtedesc, addr, retaddr);
                    }
                    ...
                    if (unlikely((info.flags | info2.flags) & TLB_WATCHPOINT)) {
                        cpu_check_watchpoint(env_cpu(env), addr,
                                             msize, info.attrs,
                                             BP_MEM_READ, retaddr, ~0ul); // SNPS changed
                    }
                    if (mtedesc && arm_tlb_mte_tagged(&info.attrs)) {
                        mte_check1_(env, mtedesc, addr, retaddr);
                    }
    ...
    do {
        ...
        do {
            ...
            if (likely((pg >> (reg_off & 63)) & 1)) {
                ...
                if (unlikely(info.flags & TLB_WATCHPOINT)) {
                    cpu_check_watchpoint(env_cpu(env), addr, msize,
                                         info.attrs, BP_MEM_WRITE, retaddr, ~0ul); // SNPS changed
                }
